...
public class CourseController {
    ...
    public ResponseEntity<?> deleteCourse(@PathVariable Long courseId, Authentication authentication) {
        ...
        try {
            ...
            // Check if course has active exams
            if (course.getExams() != null && !course.getExams().isEmpty()) {
                long activeExams = course.getExams().stream()
                    .filter(exam -> exam.isIsactive())
                    .count();
                
                if (activeExams > 0) {
                    logger.warn("Course deletion failed - Course {} has {} active exams", courseId, activeExams);
                    return ResponseEntity.badRequest().body(Map.of(
                        "error", "Cannot delete course with active or scheduled exams. Please complete or cancel all exams first.",
            ...
            // Delete the course (this will cascade delete enrollments and exams due to JPA relationships)
